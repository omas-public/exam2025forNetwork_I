...
```txt
解答欄
www.it-college.ac.jp.	3340	IN	A	163.43.87.195
...
HTTP/2 200 
server: nginx
date: Wed, 26 Nov 2025 05:24:31 GMT
...
```txt
解答欄  
nc 192.168.122 12345 < /etc/passwd
...
sudo ip netns exec p1 ip link set lo up
...
sudo ip netns exec p2 ip link set lo up
...
```txt
解答欄  
TCP/IPについてわかることができました。
ネットワーク構築やPingによる疎通確認を通して、通信の仕組みやルーティングの重要性を体感できました。
wiresharkの使い方もわかることができました。難しかったので、もっと使いこなせるようにしたいです。 
