...
HTTP/2 200 
server: nginx
date: Wed, 26 Nov 2025 05:26:11 GMT
...
```bash
解答欄
sudo ip netns exec p1 ip addr add 172.31.0.65/26 dev p1
sudo ip route add 172.31.0.128/26 via 172.31.0.126
sudo ip route add 172.31.0.64/26 via 172.31.0.126
```
#### r1の設定

```bash
解答欄  
sudo ip netns exec r1 ip addr add 172.31.0.126/26 dev r1
sudo ip route add 172.31.0.192/26 via 172.31.0.190
```
#### r2の設定   

```bash
解答欄  
sudo ip netns exec r2 ip addr add 172.31.0.190/26 dev r2
sudo ip route add 172.31.0.64/26 via 172.31.0.129
```
#### p2の設定   

```bash
解答欄  
sudo ip netns exec p2 ip addr add 172.31.0.193/26 dev p2
sudo ip route add 172.31.0.128/26 via 172.31.0.254
sudo ip route add 172.31.0.64/26 via 172.31.0.254
```
## 問題 6　この講義で学んだことについて自由に記述せよ

```txt
解答欄  
ネットワークのことがちょっとだけわかった。
```
