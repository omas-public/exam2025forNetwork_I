...
HTTP/2 200 
server: nginx
date: Wed, 26 Nov 2025 05:21:22 GMT
...
```txt
解答欄  
nc 172.16.41.72 55555 < /etc/passwd
```
...
```bash
解答欄
ip netms exec p1 bash
ip link set lo up
ip addr add 172.31.0.65/26 dev veth-p1-r1
ip route add 172.31.0.128/26 via 172.31.0.65
ip route add 172.31.0.192/26 via 172.31.0.65
exit
```
#### r1の設定
...
ip link set lo up  
ip addr add 172.31.0.126/26 dev veth-r1-p1
ip addr add 172.31.0.129/26 dev veth-r1-r2
...
ip link set lo up
ip addr add 172.31.0.190/26 dev veth-r2-r1
ip addr add 172.31.0.254/26 dev veth-r2-p2
sysctl net.ipv4.ip_forward=1
exit
```
#### p2の設定   
...
```bash
解答欄
ip netns exec p2 bash 
ip link set lo up 
ip addr add 172.31.0.193/26 dev veth-p2-r2
ip route add 172.31.0.64/26 via 172.31.0.193
ip route add 172.31.0.128/26 via 172.31.0.193
exit
```
