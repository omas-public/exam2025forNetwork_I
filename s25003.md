...
HTTP/2 200 
server: nginx
date: Wed, 26 Nov 2025 05:16:53 GMT
...
送信元: 
c0 a8 14 e3 (192.168.20.227)

宛先:
a3 2b 57 c3 (163.43.87.195)
...
sudo ip netns exec p1 ip addr add 172.31.0.129/26 dev p1-r1
sudo ip netns exec p1 ip r add 172.31.0.192/26 via 172.31.0.190
...
sudo ip netns exec r1 ip addr add 172.31.0.190/26 dev r1-p1
sudo ip netns exec r1 ip addr add 172.31.0.65/26 dev r1-r2
...
sudo ip netns exec p2 ip addr add 172.31.0.254/26 dev p2-r2
sudo ip netns exec p2 ip r add 172.31.0.192/26 via 172.31.0.254
