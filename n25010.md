...
```txt
解答欄
dig @m.root-servers.net. www.it-college.ac.jp
...
HTTP/2 200 
server: nginx
date: Wed, 26 Nov 2025 05:25:11 GMT
...
```txt
解答欄
nc -l -p 25010 > passwd_copy
```
client側コマンド:
```txt
解答欄  
nc <server-ip> 25010 < /etc/passwd
```
...
```bash
解答欄
sudo ip nets exec p1 addr add 172.31.0.1/26
```
#### r1の設定
...
sudo ip netns exec r1 ip addr add 172.31.0.128/26 dev veth-p1-r1
sudo ip netns exec r1 ip addr add 172.31.0.131/26 dev veth-r1-r2
```
#### r2の設定   
...
sudo ip netns exec r2 ip addr add 172.31.0.192/26 dev veth-p2-r2
sudo ip netns exec r2 ip addr add 172.31.0.208/26 dev veth-r2-r1
```
#### p2の設定   

```bash
解答欄  
sudo ip nets exec p2 addr add 172.31.0.64/26
```
