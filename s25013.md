...
HTTP/2 200 
server: nginx
date: Wed, 26 Nov 2025 05:23:43 GMT
...
client側コマンド:
```txt
cat /etc/passwd | nc 192.168.122.1 12345
...
```bash
sudo ip netns exec p1 bash
sudo ip link set lo up
sudo ip a add 172.31.0.65/26 dev veth-p1-r1
sudo ip r add 172.31.0.192/26 via 172.31.0.126
exit
...
```bash
sudo ip netns eexec r1 bash
sudo ip link set lo up
sudo sysctl net.ipv4.ip_forward=1  
sudo ip a add 172.31.0.126/26 dev veth-r1-p1
sudo ip a add 172.31.0.129/26 dev veth-r1-r2
sudo ip r add 172.31.0.192/26 via 172.31.0.190
exit
```
#### r2の設定   
...
sudo ip link set lo up
sudo sysctl net.ipv4.ip_forward=1
sudo ip a add 172.31.0.190/26 dev veth-r2-r1
sudo ip a add 172.31.0.254/26 dec veth-r2-p2
...
```txt
この講義で私は理解が他の人より出来ているわけではありませんが、ネットワークが楽しいと思えるようになりました。特にIPアドレスを分割する部分は理解に苦労しましたが、同級生に教えてもらうことでふんわりと大枠はわかるようになりました。少し的はずれな気もしますが、質問することの大切さと、教えてくれえる友人のありがたみをこの授業で学びました。
