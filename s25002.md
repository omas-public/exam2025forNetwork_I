...
HTTP/2 200 
server: nginx
date: Wed, 26 Nov 2025 06:25:44 GMT
...
```txt
解答欄
nc -l ポート番号 > passwd_copy
```
client側コマンド:
```txt
解答欄  
nc serverのipアドレス ポート番号 < /etc/passwd
```
...
```bash
解答欄
ip netns exec p1 ip a add 172.31.0.64/26 dev veth-p1-r1
```
#### r1の設定

```bash
解答欄  
ip netns exec r1 ip a add 172.31.0.127/26 dev veth-r1-p1
ip netns exec r1 ip a add 172.31.0.129/26 dev veth-r1-r2
```
#### r2の設定   

```bash
解答欄  
ip netns exec r2 ip a add 172.31.0.130/26 dev veth-r2-r1
ip netns exec r2 ip a add 172.31.0.255/26 dev veth-r2-p2
```
#### p2の設定   

```bash
解答欄  
ip netns exec p2 ip a add 172.31.0.192/26 dev veth-p2-r2
```
## 問題 6　この講義で学んだことについて自由に記述せよ

```txt
解答欄  
ネットにつながるのに複数の階層を行き来し、階層によって必要な情報が違うこと
```
