...
```txt
解答欄
itcollege-ac-jp0e.mail.protection.outlook.com.
...
HTTP/2 200 
server: nginx
date: Wed, 26 Nov 2025 05:42:12 GMT
...
```txt
解答欄
nc -l -p 100 > passwd_copy
```
client側コマンド:
```txt
解答欄  
nc server側のIPアドレス  100 < /etc/passwd
```
...
```txt
解答欄  
送信元IPアドレス：192.168.20.227
宛先IPアドレス：163.43.87.195
```
...
sudo ip netns exec p1 ip route add 172.31.0.128/26 via 172.31.0.126 dev veth-p1-r1
sudo ip netns exec p1 ip route add 172.31.0.192/26 via 172.31.0.126 dev veth-p1-r1
...
sudo ip netns exec r1 ip addr add 172.31.0.190/26 dev veth-r1-r2
sudo ip netns exec r1 ip route add 172.31.0.192/26 via 172.31.0.129 dev veth-r1-r2
...
sudo ip netns exec r2 ip addr add 172.31.0.129/26 dev veth-r2-r1
sudo ip netns exec r2 ip addr add 172.31.0.254/26 dev veth-r2-p2
sudo ip netns exec r2 ip route add 172.31.0.64/26 via 172.31.0.190 dev veth-r2-r1
...
sudo ip netns exec p2 ip route add 172.31.0.128/26 via 172.31.0.254 dev veth-p2-r2
sudo ip netns exec p2 ip route add 172.31.0.64/26  via 172.31.0.254 dev veth-p2-r2
...
```txt
解答欄  
コマンドの意味や使い方を学ぶことで、ネットワークの挙動を理解する基礎がつきました。今後はセキュリティなどの分野でも活かしていきたいです
```
