...
sudo ip netns exec r1 ip route add 172.31.0.64/26 dev veth-r1-p1
sudo ip netns exec r1 ip route add 172.31.0.192/26 via 172.31.0.130 dev veth-r1-r2
```bash
解答欄  

```
#### r2の設定   
sudo ip netns exec r2 ip addr add 172.31.0.130/26 dev veth-r2-r1
sudo ip netns exec r2 ip addr add 172.31.0.193/26 dev veth-r2-p2
sudo ip netns exec r2 ip link set dev lo up
sudo ip netns exec r2 ip route add 172.31.0.128/26 dev veth-r2-r1
sudo ip netns exec r2 ip route add 172.31.0.64/26 via 172.31.0.129 dev veth-r2-r1
```bash
解答欄  

```
#### p2の設定   
sudo ip netns exec p2 ip addr add 172.31.0.194/26 dev veth-p2-r2
sudo ip netns exec p2 ip link set dev lo up
sudo ip netns exec p2 ip route add 172.31.0.64/26 via 172.31.0.193 dev veth-p2-r2
sudo ip netns exec p2 ip route add 172.31.0.128/26 via 172.31.0.193 dev veth-p2-r2
```bash
解答欄  
