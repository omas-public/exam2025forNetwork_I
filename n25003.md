...
HTTP/2 200 
server: nginx
date: Wed, 26 Nov 2025 06:08:38 GMT
...
server側コマンド:
nc -l 5555 > passwd_copy

```
client側コマンド:
```txt
解答欄  
nc server側IPアドレス 5555 < /etc/passwd
```
...
```bash
解答欄
sudo ip netns p1 bash
sudo ip a add 172.31.0.65/26 dev veth-p1-r1
sudo ip r add 172.31.0.192/26 via 172.31.0.80
exit
```
#### r1の設定
```bash
...
sudo ip a add 172.31.0.80/26 dev veth-r1-p1
sudo ip a add 172.31.0.130/26 dev veth-r1-r2
sudo ip r add 172.31.0.192/26 via 172.31.0.140
...
sudo ip a add 172.31.0.140/26 dev veth-r2-r1
sudo ip a add 172.31.0.200/26 dev veth-r2-p2
sudo ip r add 172.31.0.64/26 via 172.31.0.130
sudo sysctl net.ipv4.ip_forward=1
exit
```
#### p2の設定   
...
```bash
解答欄  
sudo ip netns exec p2 bash
sudo ip a add 172.31.0.193/26 dev veth-p2-r2
sudo ip r add 172.31.0.64/26 via 172.31.0.200
exit
```
...
```txt
解答欄  
コマンドやwiresharkを用いて通信の中を覗いたり解析することは面白かったです。
前期修了時はLinuxやPythonをどう仕事に利用していくのかいまいち想像がつきませんでしたが少しはITへの理解が進んだかなと思います。
```
