...
HTTP/2 200 
server: nginx
date: Wed, 26 Nov 2025 05:25:54 GMT
...
```txt
解答欄  
nc サーバのIPアドレス 12345 < /etc/passwd
```
...
sudo ip netns exec r1 ip route add 172.31.0.192/26 via 172.31.0.193
sudo ip netns exec r1 ip route add 172.31.0.65/26 dev veth-r1-p1
```
#### r2の設定   

```bash
解答欄  
sudo ip netns exec r2 ip addr add 172.31.0.193/26 dev veth-r2-r1
sudo ip netns exec r2 ip addr add 172.31.0.254/26 dev veth-r2-p2
sudo ip netns exec r2 ip route add 172.31.0.65/26 via 172.31.0.190
...
sudo ip netns exec p2 ip route add 172.31.0.65/26 via 172.31.0.254
