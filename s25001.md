...
HTTP/2 200 
server: nginx
date: Wed, 26 Nov 2025 05:25:56 GMT
content-type: text/html; charset=UTF-8
x-powered-by: PHP/7.4.33
link: <https://www.it-college.ac.jp/wp-json/>; rel="https://api.w.org/"
...
タイプが08 でエコーリクエスト
コードが0
pingがクライアントから送信されたが、サーバーからの応答は返ってきてない。
```
...
```bash
解答欄
sudo ip netns exec p1 ip addr add 172.31.64.1/26 dev veth-p1-r1
sudo ip netns exec p1 ip route add 172.31.128.0/30 via 172.31.64.62
sudo ip netns exec p1 ip route add 172.31.192.0/26 via 172.31.64.62
```
#### r1の設定

```bash
解答欄  
sudo ip netns exec r1 ip addr add 172.31.64.62/26 dev veth-r1-p1
sudo ip netns exec r1 ip addr add 172.31.128.1/30 dev veth-r1-r2
sudo ip netns exec r1 sysctl -w net.ipv4.ip_forward=1
sudo ip netns exec r1 ip route add 172.31.64.0/26 dev veth-r1-p1
sudo ip netns exec r1 ip route add 172.31.192.0/26 via 172.31.128.2
```
#### r2の設定   

```bash
解答欄  
sudo ip netns exec r2 ip addr add 172.31.128.2/30 dev veth-r2-r1
sudo ip netns exec r2 ip addr add 172.31.192.62/26 dev veth-r2-p2
sudo ip netns exec r2 sysctl -w net.ipv4.ip_forward=1
sudo ip netns exec r2 ip route add 172.31.192.0/26 dev veth-r2-p2
sudo ip netns exec r2 ip route add 172.31.64.0/26 via 172.31.128.1
```
#### p2の設定   

```bash
解答欄  
sudo ip netns exec p2 ip addr add 172.31.192.1/26 dev veth-p2-r2
sudo ip netns exec p2 ip route add 172.31.128.0/30 via 172.31.192.62
sudo ip netns exec p2 ip route add 172.31.64.0/26 via 172.31.192.62
```
## 問題 6　この講義で学んだことについて自由に記述せよ

```txt
解答欄  
同じコンピューターでネットワーク間の繋がりを学べる授業だった。I期、II期でSEA/J
やLINUXで出てきた単語やコマンド、tcp,udpなどをdns,ファイル書き込みなどを使って学習し、復習にもなるいい機会だった。
```
