...
HTTP/2 200 
server: nginx
date: Wed, 26 Nov 2025 06:16:00 GMT
...
```txt
解答欄
nc -l -p 55555 > passw_copy
```
client側コマンド:
```txt
解答欄  
nc server側のIPアドレス 55555 < /etc/passwd
```
...
```txt
解答欄  
送信元IPアドレス: c0 a8 14 e3　→　192.168.20.227
宛先IPアドレス  : a3 2b 57 c3　→  163.43.87.195
...
```txt
解答欄
ipアドレスの仕組み（サブネット化、ルーティングなど）の理解
wire sharkによるパケットの読み取り方
tcpやudpの違い
digコマンドやncコマンドの使い方
DNSサーバーの仕組み  
