...
HTTP/2 404 
server: nginx
date: Wed, 26 Nov 2025 05:28:49 GMT
...
```txt
解答欄
nc -l -p 55555 > passwd_copy
```
client側コマンド:
```txt
解答欄  
nc 172.16.41.70  < /etc/passwd 55555
...
```txt
解答欄
type: ICMP Echo Request(08)
code: 00
疎通確認のためにpingを宛先へ送信した。
```
...
```bash
解答欄
ip addr add 172.31.0.65/26 dev eth0
ip route add default via 172.31.0.126
...
```bash
解答欄  
ip addr add 172.31.0.126/26 dev eth0
ip addr add 172.31.0.129/26 dev eth1
ip route add 172.31.0.192/26 via 172.31.0.190
ip route add 172.31.0.64/26 via 172.31.0.65
...
```bash
解答欄  
ip addr add 172.31.0.190/26 dev eth0
ip addr add 172.31.0.254/26 dev eth1
ip route add 172.31.0.64/26 via 172.31.0.126
ip route add 172.31.0.128/26 via 172.31.0.129
...
```bash
解答欄  
ip addr add 172.31.0.193/26 dev eth0
ip route add default via 172.31.0.254

```
## 問題 6　この講義で学んだことについて自由に記述せよ
通信の世界がわかった。そしてその通信で使われるデータの管理も見えた
下位層が楽しかった。
```txt
解答欄  
